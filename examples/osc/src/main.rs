use audioviz::io::{Input, InputController, Output};
use audioviz::io::Device;
use audioviz::utils::seperate_channels;
use audioviz::processor::{MultiChannelProcessor, Plugin, Lowpass};
...
impl Data {
    ...
    fn get(&mut self) -> Vec<[f32; 2]> {
        ...
        // get new data
        let data = self.input_controller.pull_data();
        if !data.is_empty() {
            // cap
...
async fn main() {
    ...
    let mut audio_output = Output::new();
    let (_channel_count, _sampling_rate, _output_controller) = audio_output.init(&Device::DefaultOutput).unwrap();
    ...
    loop {
        ...
        for l_r in l_r.iter() {
            ...
            alpha += a;
            points.push([p1 * 0.95 + 0.5, p2 * -0.95 + 0.5]);
        }
